	...
	if( $_POST ):
		if( isset($_POST["save-options"]) ):
			...
			foreach($_POST as $value):
				...
				if( $value[1] == 'file-link' ):
					$site_url = get_site_url();
					$option_text = str_replace($site_url, '', $option_text);
				endif;
			...
			echo '<div class="add-more-options-container">';
				...
				echo '<div class="option-type-select">';
					echo '<select name="option-type">';
						...
						echo '<option value="file-link">File Link</option>';
					echo '</select>';
				echo '</div>';
				...
				foreach( $current_wb_options as $option ):
					...
					echo '<div class="row">';
						echo '<div class="row-title">';
							...
							if( $option_value["option_type"] == 'file-link' ):
								echo '<div class="option-shortcode">[wbo_file_link option_name="'.$option->option_name.'"]YOUR CONTENT HERE[/wbo_file_link]</div>';
							else:
						...
						echo '<div class="row-content">';
							...
							elseif( $option_value["option_type"] == 'wysiwyg' ):
								...
								echo '</div>';
							elseif( $option_value["option_type"] == 'file-link' ):
								echo '<div class="option-value">';
									echo '<input type="hidden" name="'.$option->option_name.'[]" value="'.$option_value["option_title"].'" />';
									echo '<input type="hidden" name="'.$option->option_name.'[]" value="'.$option_value["option_type"].'" />';
									echo '<input type="text" name="'.$option->option_name.'[]" value="'.$option_value["option_text"].'" readonly="readonly" class="file-link-link" />';
									echo '<a href="#" class="change-file-link button button-primary button-large">Change File Link</a>';
								echo '</div>';
							endif;
						echo '</div>';
					echo '</div>';
...
function get_white_bracket_documntation(){
	...
	$output = '<h4>Standard shortcodes</h4>';
	$output .= '<p>[wbo option_name="wb_XXXX"]</p>';
	$output .= '<p>[wbo_file_link option_name="wb_XXXX"]XXXX[/wbo_file_link]</p>';

	$output .= '<h4>Remove p tags</h4>';
	$output .= '<p>[wbo option_name="wb_XXXX" wpautop="no"]</p>';
	$output .= '<p>[wbo_file_link option_name="wb_XXXX" wpautop="no"]XXXX[/wbo_file_link]</p>';

	$output .= '<h4>Open fle links in a new tab</h4>';
	$output .= '<p>[wbo_file_link option_name="wb_XXXX" new_tab="yes"]XXXX[/wbo_file_link]</p>';

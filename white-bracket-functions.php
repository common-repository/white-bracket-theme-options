...
function get_white_bracket_file_link_option( $atts, $content = null ){
    $wb_atts = shortcode_atts( array(
        'option_name'   => '',
        'wpautop'       => 'yes',
        'new_tab'       => 'no'
    ...
    if( $wb_atts_check !== false ):
        ...
        $option_data = unserialize(get_option($wb_atts["option_name"]));
        $link = $option_data["option_text"];

        if( $wb_atts["new_tab"] == 'yes' ):
            $target = "_blank";
        else:
            $target = "_self";
        endif;

        $option_content = '<a href="'.$link.'" target="'.$target.'">';
            $option_content .= $content;
        $option_content .= '</a>';

        if( $wb_atts["wpautop"] == 'yes' ):
            $option_content = wpautop( $option_content );
        endif;
    else:
        $option_content = wpautop('White Bracket option does not exist.');
    endif;
...
add_shortcode( 'wbo_file_link', 'get_white_bracket_file_link_option' );
